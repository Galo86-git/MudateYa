// api/cron-seguimiento-remax.js
// Seguimiento escalonado a las oficinas RE/MAX que ya recibieron la
// propuesta (enviar-propuesta-remax.js) y, muchas, el primer recordatorio
// (recordatorio-remax.js) — cadencia y cruce contra altas reales en
// ./_seguimiento.js (ver ahí el detalle: 7 días hasta el touch 3, cada 10
// días de ahí en más, sin tope).
//
// SIEMBRA: el estado arranca a partir de remax-propuesta:enviados (touch 1,
// mandado 2026-08-05) y remax-recordatorio:enviados (touch 2, mandado
// 2026-08-10) — así el cron no repite ningún toque que ya salió a mano.
//
// CRUCE CONTRA ALTA: una oficina se considera "convertida" si algún asesor
// de remax:asesores tiene .oficina == el nombre de esa oficina (normalizado,
// sin acentos/mayúsculas — remax.js valida ese campo contra el padrón
// oficial, así que el match es exacto salvo por tildes/caja).
//
// COPY: touch 2 y 3 reusan el mismo layout de recordatorio-remax.js (incluido
// el link al PDF ya generado por ese endpoint — no se duplica el generador
// de PDF acá). Touch 4+ es un mail corto y espaciado, para no cansar la
// bandeja de entrada de una oficina que simplemente no le interesó.
//
...
const { elegiblesHoy, marcarEnviado, getJSON, redisCall } = require('./_seguimiento');
const OFICINAS = require('./_remax-oficinas');
...
var CLAVE_ESTADO = 'remax-seguimiento:estado';
// Fechas reales de las campañas previas (ver manifiesto de envíos) — para que
// el touch 3 no espere de más ni de menos respecto de cuándo salió el touch 2 real.
var FECHA_TOUCH1 = '2026-08-05';
var FECHA_TOUCH2 = '2026-08-10';

// ── Oficinas ya en charla directa 1 a 1 por mail — no entran en el
//    seguimiento automático (mismo criterio que recordatorio-remax.js).
//    Cargar acá a mano si aparece algún caso nuevo. ──
var EXCLUIR = ['197@remax.com.ar', 'platino3@remax.com.ar'];
var UNIVERSO = OFICINAS.filter(function (o) { return EXCLUIR.indexOf(o.e) === -1; });

// ── Prefetch de todos los asesores RE/MAX una sola vez por corrida — barato
//    (2 llamadas a Redis) contra hacer 1 llamada por oficina candidata. ──
var _cacheAsesores = null;
async function oficinasConAlta() {
  if (_cacheAsesores) return _cacheAsesores;
  var ids = (await getJSON('remax:asesores')) || [];
  var set = {};
  for (var i = 0; i < ids.length; i++) {
    var a = await getJSON('remax:asesor:' + ids[i]);
    if (a && a.oficina) set[normOficina(a.oficina)] = 1;
  }
  _cacheAsesores = set;
  return set;
}
async function estaConvertido(oficina) {
  var set = await oficinasConAlta();
  return !!set[normOficina(oficina.n)];
}

function linkPDF(email) {
  // Reusa el endpoint público que ya sirve el PDF (recordatorio-remax.js) —
  // no se genera un PDF nuevo acá.
...
function emailTouch(o, touch) {
  ...
  var linkB = linkBaja(o.e, 'remax-seguimiento');
  var linkP = linkPDF(o.e);
  var linkRegistro = 'https://mudateya.ar/remax-registro.html';
...
function payloadDe(o, touch) {
  ...
  var link = linkBaja(o.e, 'remax-seguimiento');
  return {
...
module.exports = async function handler(req, res) {
  ...
  try {
    var semilla = {
      touch1: (await getJSON('remax-propuesta:enviados')) || [],
      touch2: (await getJSON('remax-recordatorio:enviados')) || [],
      fechaTouch1: FECHA_TOUCH1,
      fechaTouch2: FECHA_TOUCH2
    };

    var res1 = await elegiblesHoy({
      clave: CLAVE_ESTADO,
      universo: UNIVERSO,
      semilla: semilla,
      estaConvertido: estaConvertido
    });
    ...
    console.error('Error en cron-seguimiento-remax:', e.message);
