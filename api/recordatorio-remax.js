// api/recordatorio-remax.js
// ── ONE-OFF (solo admin) ──
// Recordatorio a las oficinas RE/MAX de AMBA (127) que ya recibieron la
// propuesta de MudateYa y todavía no se sumaron ningún asesor. Mismo público
// que enviar-propuesta-remax.js (api/_remax-oficinas.js), copy más corto y
// directo ("che, ¿lo viste?"), mismo link a /remax-registro.html.
//
// EXCLUYE a mano las oficinas con las que ya hay charla directa por mail
// (respondieron y Galo les está contestando 1 a 1) — no tiene sentido
// mandarles un recordatorio genérico en medio de esa conversación.
//
// ENVÍO: batch de Resend (hasta 100 por llamada).
// IDEMPOTENCIA: Redis (clave remax-recordatorio:enviados) — propia, separada
// de remax-propuesta:enviados, para no pisar el registro de la campaña 1.
//
...
const TODAS_OFICINAS = require('./_remax-oficinas');
...
var CLAVE_ENVIADOS = 'remax-recordatorio:enviados';

// ── Ya en charla directa por mail — no les mandamos el recordatorio genérico ──
var EXCLUIR = ['197@remax.com.ar', 'platino3@remax.com.ar'];
...
//    no se adjunta al mail: un adjunto igual a 138 direcciones del mismo
//    dominio externo en una corrida corta es justo el patrón que el filtro
//    corporativo de RE/MAX (Microsoft 365 / Google Workspace) puede leer
//    como mailing masivo y empezar a cuarentenar. ──
function linkPDF(email) {
  return 'https://mudateya.ar/api/recordatorio-remax?pdf=1&e=' + encodeURIComponent(email);
}

function emailRecordatorio(o) {
  var nm = esc(o.n || 'tu oficina');
  var linkB = linkBaja(o.e, 'remax-recordatorio');
  var linkP = linkPDF(o.e);
  // ── Córdoba/Rosario (o.nueva) nunca recibieron la propuesta original —
  //    para ellas esto es primer contacto, no un recordatorio. ──
  var subject = o.nueva
    ? nm + ': te presentamos MudateYa, un beneficio para tus asesores'
    : nm + ': ¿ya le contaste a tus asesores sobre MudateYa?';
  var introHtml = o.nueva
    ? '<p style="color:#475569;font-size:14px;line-height:1.7;margin:0 0 18px">Te escribimos desde <strong>MudateYa</strong>, la plataforma argentina de mudanzas pensada como valor agregado para inmobiliarias y asesores. Queríamos invitar a ' + nm + ' a sumarse como aliados.</p>'
    : '<p style="color:#475569;font-size:14px;line-height:1.7;margin:0 0 18px">La semana pasada te contamos sobre <strong>MudateYa</strong> como un beneficio extra para tus asesores. Como no tuvimos noticias, te lo volvemos a acercar por si se pasó entre tantos mails.</p>';
  var introText = o.nueva
    ? 'Te escribimos desde MudateYa, la plataforma argentina de mudanzas pensada como valor agregado para inmobiliarias y asesores. Queríamos invitar a ' + nm + ' a sumarse como aliados.\n\n'
    : 'La semana pasada te contamos sobre MudateYa como un beneficio extra para tus asesores. Como no tuvimos noticias, te lo volvemos a acercar por si se pasó entre tantos mails.\n\n';
  return {
    subject: subject,
    html:
      '<div style="font-family:Inter,Arial,sans-serif;max-width:560px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #E2E8F0">' +
        ...
        '<div style="padding:28px">' +
          ...
          '<div style="text-align:center;margin:20px 0">' +
            '<a href="https://mudateya.ar/remax-registro.html" style="display:inline-block;background:#22C36A;color:#fff;padding:14px 28px;border-radius:10px;font-weight:700;font-size:15px;text-decoration:none">Sumar a mis asesores →</a>' +
          '</div>' +
    ...
    text:
      ...
      '- El registro es individual, 30 segundos: https://mudateya.ar/remax-registro.html\n\n' +
      ...
      'Recibís este correo como oficina RE/MAX. Para darte de baja: ' + linkB
  };
}
...
function payloadDe(o) {
  ...
  var link = linkBaja(o.e, 'remax-recordatorio');
  return {
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    console.error('Error en recordatorio-remax:', e.message);
