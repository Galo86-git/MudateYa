// api/enviar-propuesta-keller.js
// ── ONE-OFF (solo admin) ──
// Primer contacto con los Market Centers de Keller Williams en AMBA +
// Córdoba (api/_keller-oficinas.js, 10). A diferencia de RE/MAX, acá cada
...
const TODAS_OFICINAS = require('./_keller-oficinas');
...
var CLAVE_ENVIADOS = 'keller-propuesta:enviados';
...
function emailPropuesta(o) {
  ...
  var linkB = linkBaja(o.e, 'keller-propuesta');
  var linkP = linkPDF(o.e);
  return {
    subject: nm + ': un beneficio de posventa para tu red de asesores',
    html:
...
// ── PDF institucional, personalizado con el nombre del Market Center.
//    Informativo — sin botón de CTA. Mismo patrón que recordatorio-remax.js. ──
function generarPDFOficinaBase64(o) {
  const PDFDocument = require('pdfkit');
  return new Promise(function(resolve, reject) {
    try {
      var nm = o.n || 'tu Market Center';
      ...
      p('El mercado de mudanzas en Argentina sigue siendo, en su mayoría, informal: se resuelve con recomendaciones sueltas o búsquedas en redes, sin verificación ni respaldo. Los Market Centers y asesores que incorporan hoy un servicio como MudateYa a su proceso de posventa son quienes construyen primero ese hábito con sus clientes, y capturan el beneficio antes de que se vuelva un estándar del sector.');
...
function payloadDe(o) {
  ...
  var link = linkBaja(o.e, 'keller-propuesta');
  return {
...
module.exports = async function handler(req, res) {
  ...
  if (req.query && req.query.pdf === '1') {
    ...
    if (!oficina) return res.status(404).json({ error: 'Market Center no encontrado' });
  ...
  try {
    if (testTo) {
      ...
      var rt = await resend.emails.send(payloadDe({ n: 'KW Norte', e: testTo }));
    ...
    console.error('Error en enviar-propuesta-keller:', e.message);
