  ...
  if(req.method==='POST'){
    ...
    if(!body.accion && body.clienteEmail){
      ...
      try {
        ...
        if(admin) await resend.emails.send({ from:'MudateYa <noreply@mudateya.ar>', to:admin, subject:`💸 Transferencia pendiente — ${clienteNombre} · ${monto}`, html:`<div style="font-family:Arial;background:#0D1410;color:#E8F5EE;padding:24px;border-radius:12px"><h2 style="color:#FFB300">💸 Transferencia pendiente</h2><p>Cliente: <strong>${clienteNombre}</strong> (${clienteEmail})</p><p>Mudancero: ${mudancero}</p><p>Ruta: ${desde} → ${hasta}</p><p>Monto: <strong style="color:#FFB300">${monto}</strong></p><p>Fecha: ${fecha}</p><a href="${process.env.SITE_URL || 'https://mudateya.ar'}/admin" style="background:#FFB300;color:#041A0E;padding:10px 20px;border-radius:8px;text-decoration:none;font-weight:700">Ver en Admin →</a></div>` });
        if(clienteEmail) await resend.emails.send({ from:'MudateYa <noreply@mudateya.ar>', to:clienteEmail, subject:'Recibimos tu comprobante — MudateYa', html:`<div style="font-family:Arial;background:#0D1410;color:#E8F5EE;padding:24px;border-radius:12px"><h2 style="color:#22C36A">✅ Comprobante recibido</h2><p>Hola <strong>${clienteNombre}</strong>, recibimos tu comprobante de <strong>${monto}</strong>. Lo validamos en las próximas 2hs hábiles.</p></div>` });
    ...
    if(body.id && body.accion){
      ...
      try {
        ...
        if(process.env.RESEND_API_KEY && t.clienteEmail){
          const resend = new Resend(process.env.RESEND_API_KEY);
          if(accion==='confirmar') await resend.emails.send({ from:'MudateYa <noreply@mudateya.ar>', to:t.clienteEmail, subject:'✅ ¡Tu reserva está confirmada! — MudateYa', html:`<div style="font-family:Arial;background:#0D1410;color:#E8F5EE;padding:24px;border-radius:12px"><h2 style="color:#22C36A">✅ Reserva confirmada</h2><p>Hola <strong>${t.clienteNombre}</strong>, validamos tu transferencia de <strong>${t.monto}</strong>. Tu mudanza con <strong>${t.mudancero}</strong> está confirmada.</p><a href="${process.env.SITE_URL || 'https://mudateya.ar'}/mi-mudanza" style="background:#22C36A;color:#041A0E;padding:10px 20px;border-radius:8px;text-decoration:none;font-weight:700">Ver mi mudanza →</a></div>` });
          else await resend.emails.send({ from:'MudateYa <noreply@mudateya.ar>', to:t.clienteEmail, subject:'Información sobre tu transferencia — MudateYa', html:`<div style="font-family:Arial;background:#0D1410;color:#E8F5EE;padding:24px;border-radius:12px"><h2>Transferencia no validada</h2><p>Hola <strong>${t.clienteNombre}</strong>, no pudimos validar tu transferencia. Respondé este email para resolverlo.</p></div>` });
        }
        return res.status(200).json({ ok:true });
