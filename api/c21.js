// api/c21.js
//
// Endpoint del canal CENTURY 21. Da de alta asesores de C21 y les genera un
// link único de derivación. Calcado de api/mudafy.js / api/independientes.js.
//
// MODELO (idéntico a api/mudafy.js):
// - Todos los asesores quedan bajo el partner "c21", con su propio {codigo}
//   para atribuir su comisión individual.
...
//   Crear en el panel de Inmobiliarias el slug "c21" con su comisión.
//   Sin ese registro en `inmobiliaria:c21`, cotizaciones.js no reconoce el
//   partner y la atribución se pierde en silencio.
//
// REDIS:
//   c21:asesor:{codigo}  → { codigo, nombre, email, whatsapp, oficina, origen, activo, createdAt }
//   c21:asesores         → array con todos los códigos (índice para listar)
//   c21:email:{email}    → codigo (alta idempotente por email)
//
// ACTIONS:
...
async function generarCodigo(nombre) {
  ...
  for (var intento = 0; intento < 6; intento++) {
    ...
    var existe = await getJSON('c21:asesor:' + codigo);
...
async function agregarAlIndice(codigo) {
  var lista = (await getJSON('c21:asesores')) || [];
  if (lista.indexOf(codigo) === -1) {
    lista.push(codigo);
    await setJSON('c21:asesores', lista);
  }
}
...
// ── Escape de HTML para todo lo que venga del listado (nombre, oficina, etc.)
...
//   importado=true  → lo dio de alta CENTURY 21 desde el listado. No pidió
//                     nada, así que el mail explica qué es MudateYa y cómo
//                     darse de baja.
// opts: { nombre, email, whatsapp, oficina, codigo, link, importado, to, prueba }
...
async function enviarBienvenida(opts) {
  ...
  // disponible vía /api/c21?action=qr para quien lo necesite desde el
  // admin, y para "se me perdió el link" ahora está Emi por WhatsApp.
  var asunto = importado
    ? 'CENTURY 21 te dio de alta en MudateYa · tu link'
    : '✅ Ya sos aliado de MudateYa a través de CENTURY 21 · tu link';
  ...
  var intro = importado
    ? '<p style="color:#4B5563;line-height:1.6;font-size:14.5px;margin:0 0 14px"><strong>CENTURY 21 sumó a sus asesores a MudateYa</strong> y ya te dimos de alta. MudateYa es la plataforma donde tus clientes consiguen mudanceros verificados, con presupuestos para comparar y seguimiento de la mudanza.</p>'
    ...
    : '<p style="color:#4B5563;line-height:1.6;font-size:14.5px;margin:0 0 20px">Ya sos aliado de MudateYa a través de CENTURY 21. Este es tu <strong>link único</strong>: compartilo con tus clientes cuando cierren una operación y ellos consiguen mudanceros verificados.</p>';

  // List-Unsubscribe SOLO en la variante importada: el asesor no pidió nada,
  // se lo dio de alta CENTURY 21 — mismo criterio que mudafy.js.
  var headersMail = importado
    ? {
        'List-Unsubscribe': '<mailto:hola@mudateya.ar?subject=BAJA>, <' + linkBaja(destino, 'c21-importado') + '>',
        ...
        <div style="padding:28px">
          ...
          <div style="background:#F5F8FC;border:1px solid #E5ECF6;border-radius:10px;padding:14px 18px;margin:0 0 20px">
            ...
            <table style="font-size:14px;color:#0F1419;line-height:1.8">
              ...
              ${oficina ? '<tr><td style="color:#64748B;padding-right:12px">Oficina</td><td style="font-weight:600">' + escHtml(oficina) + '</td></tr>' : ''}
          ...
          <div style="background:#FAF8F3;border-left:3px solid #BEAF87;border-radius:8px;padding:12px 16px;margin:0 0 8px">
...
function normalizarFila(fila) {
  ...
  var oficina  = (typeof fila.oficina === 'string')  ? fila.oficina.trim().slice(0, 80)                     : '';
  ...
  return { ok: true, asesor: { nombre: nombre, email: email, whatsapp: whatsapp, oficina: oficina } };
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'registrar' && req.method === 'POST') {
      ...
      var oficina      = (typeof body.oficina === 'string')      ? body.oficina.trim().slice(0, 80)      : '';
      ...
      var emailKey = 'c21:email:' + email.toLowerCase();
      var existenteCod = await getJSON(emailKey);
      if (existenteCod) {
        var existente = await getJSON('c21:asesor:' + existenteCod);
        if (existente && existente.activo !== false) {
          var linkExist = LINK_BASE + '?asesor=' + encodeURIComponent(existente.codigo);
          return res.status(200).json({ ok: true, yaExistia: true, codigo: existente.codigo, link: linkExist, oficina: existente.oficina || '' });
        }
      }
      ...
      await setJSON('c21:asesor:' + codigo, asesor);
      ...
      try {
        await enviarBienvenida({
          nombre: nombre, email: email, whatsapp: whatsapp, oficina: oficina,
          codigo: codigo, link: link, importado: false
        });
      } catch (mailErr) {
        console.warn('No se pudo enviar el mail de alta C21:', mailErr.message);
      }

      return res.status(200).json({ ok: true, codigo: codigo, link: link, oficina: oficina });
    }

    // ── ADMIN: ALTA MASIVA desde el listado que manda CENTURY 21 ───────────
    //
    // La UI (/c21-importar) parsea el CSV, deja corregir las filas con error y
    // manda de a lotes chicos (5) para no acercarse al timeout de la función:
    // cada alta son 4 llamadas a Redis + un mail con QR adjunto.
    //
    // BODY:
    //   asesores    → [{ fila, nombre, email, whatsapp, oficina }]  (máx 20)
    ...
    if (action === 'importar' && req.method === 'POST') {
      ...
      for (var fi = 0; fi < filas.length; fi++) {
        ...
        try {
            ...
            if (enviarMail) {
              try {
                await enviarBienvenida({
                  ...
                  oficina: aImp.oficina, codigo: codPrueba, link: linkPrueba,
          ...
          var emailKeyImp = 'c21:email:' + aImp.email;
          var codExistente = await getJSON(emailKeyImp);
          var yaEsta = codExistente ? await getJSON('c21:asesor:' + codExistente) : null;
            ...
            if (enviarMail && reenviar) {
              try {
                await enviarBienvenida({
                  ...
                  oficina: yaEsta.oficina, codigo: yaEsta.codigo, link: linkYa,
                  importado: true
                });
          ...
          var regImp = {
            ...
            oficina: aImp.oficina,
            origen: origenImp,
            activo: true,
            createdAt: new Date().toISOString(),
            importadoPor: quien
          };
          await setJSON('c21:asesor:' + codImp, regImp);
          ...
          if (enviarMail) {
            try {
              await enviarBienvenida({
                nombre: aImp.nombre, email: aImp.email, whatsapp: aImp.whatsapp,
                oficina: aImp.oficina, codigo: codImp, link: linkImp,
                importado: true
              });
    ...
    if (action === 'reenviar' && req.method === 'POST') {
      ...
      var aRe = await getJSON('c21:asesor:' + codRe);
      ...
      try {
        await enviarBienvenida({
          ...
          oficina: aRe.oficina, codigo: aRe.codigo, link: linkRe,
          importado: true
        });
      } catch (eRe) {
    ...
    if (action === 'qr' && req.method === 'GET') {
      ...
      var aQr = await getJSON('c21:asesor:' + codQr);
      ...
      var png = await QRCode.toBuffer(linkQr, { type: 'png', width: 600, margin: 1, color: { dark: '#252526', light: '#FFFFFF' } });
    ...
    if (action === 'eliminar' && req.method === 'POST') {
      ...
      var aDel = await getJSON('c21:asesor:' + codDel);
      await redisCall('del', ['c21:asesor:' + codDel]);
      var idxDel = (await getJSON('c21:asesores')) || [];
      await setJSON('c21:asesores', idxDel.filter(function(c){ return c !== codDel; }));
      if (aDel && aDel.email) await redisCall('del', ['c21:email:' + aDel.email.toLowerCase()]);
    ...
    if (action === 'eliminar-todos' && req.method === 'POST') {
      if (!esAdmin(req)) return res.status(401).json({ error: 'No autorizado' });
      var idsAll = (await getJSON('c21:asesores')) || [];
      for (var k = 0; k < idsAll.length; k++) {
        var aAll = await getJSON('c21:asesor:' + idsAll[k]);
        await redisCall('del', ['c21:asesor:' + idsAll[k]]);
        if (aAll && aAll.email) await redisCall('del', ['c21:email:' + aAll.email.toLowerCase()]);
      }
      await setJSON('c21:asesores', []);
    ...
    if (action === 'obtener' && req.method === 'GET') {
      ...
      var a = await getJSON('c21:asesor:' + cod);
      ...
      return res.status(200).json({ codigo: a.codigo, nombre: a.nombre, oficina: a.oficina, inmobiliaria: 'CENTURY 21' });
    }
    ...
    if (action === 'listar' && req.method === 'GET') {
      ...
      for (var i = 0; i < ids.length; i++) {
        var m = await getJSON('c21:asesor:' + ids[i]);
