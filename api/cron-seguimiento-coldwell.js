// api/cron-seguimiento-coldwell.js
// Seguimiento escalonado a las oficinas de Coldwell Banker
// (api/_coldwell-oficinas.js, 28) — cadencia y cruce contra altas reales en
// ./_seguimiento.js: 7 días hasta el touch 3, cada 10 días de ahí en más,
// sin tope. Primer seguimiento automático de esta red (no tenía
// recordatorio previo).
//
// SIEMBRA: arranca desde coldwell-propuesta:enviados (touch 1, mandado
// 2026-08-11). No hay touch 2 previo.
//
// CRUCE CONTRA ALTA: a diferencia de las otras 3 redes (alta individual de
// asesor), acá el CTA manda a inmobiliarias-registro.html — la OFICINA se
// da de alta como partner independiente con su propio slug
// (inmobiliaria:{slug}, ver api/inmobiliarias.js), no cada asesor suelto.
// El match es contra inmobiliarias:lista: por email de contacto exacto
// (data.contactoEmail) o, si no matchea, por nombre normalizado (fuzzy).
//
// COPY: touch 2 y 3 linkean al PDF que ya sirve enviar-propuesta-coldwell.js
...
const OFICINAS = require('./_coldwell-oficinas');
...
var CLAVE_ESTADO = 'coldwell-seguimiento:estado';
var FECHA_TOUCH1 = '2026-08-11';
...
async function estaConvertido(oficina) {
  ...
  var nombreObjetivo = normOficina(oficina.n).replace(/^coldwell banker /, '');
  for (var i = 0; i < inmos.length; i++) {
    if (inmos[i].email && inmos[i].email === emailObjetivo) return true;
    if (nombreObjetivo && contieneOSonContenidos(inmos[i].nombre, nombreObjetivo)) return true;
  }
  return false;
}
...
function emailTouch(o, touch) {
  ...
  var linkB = linkBaja(o.e, 'coldwell-seguimiento');
  ...
  if (touch === 2) {
    ...
    introHtml = '<p style="color:#475569;font-size:14px;line-height:1.7;margin:0 0 18px">Hace una semana te escribimos sobre <strong>MudateYa</strong>: un beneficio de posventa para sumar a todo el equipo de asesores de ' + nm + ', sin costo para la oficina.</p>';
    introText = 'Hace una semana te escribimos sobre MudateYa: un beneficio de posventa para sumar a todo el equipo de asesores de ' + nm + ', sin costo para la oficina.\n\n';
  } else if (touch === 3) {
  ...
  return {
      ...
      '<div style="font-family:Inter,Arial,sans-serif;max-width:560px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #E2E8F0">' +
        ...
        '<div style="padding:28px">' +
          ...
          '<div style="text-align:center;margin:20px 0">' +
            '<a href="' + linkRegistro + '" style="display:inline-block;background:#22C36A;color:#fff;padding:14px 28px;border-radius:10px;font-weight:700;font-size:15px;text-decoration:none">Dar de alta la oficina →</a>' +
          '</div>' +
          ...
          '<p style="color:#475569;font-size:13px;line-height:1.7;margin:0">Si ya la diste de alta, ignorá este mail. Cualquier duda, respondan a este mismo mail.</p>' +
    ...
    text:
      'Hola equipo de ' + nm + ',\n\n' + introText +
      'Dar de alta la oficina: ' + linkRegistro + '\n\n' +
      (touch >= 3 ? 'Propuesta completa en PDF: ' + linkP + '\n\n' : '') +
      'Si ya la diste de alta, ignorá este mail. Cualquier duda, respondan a este mismo mail.\n\n' +
...
function payloadDe(o, touch) {
  ...
  var link = linkBaja(o.e, 'coldwell-seguimiento');
  return {
...
module.exports = async function handler(req, res) {
  ...
  try {
    var semilla = {
      touch1: (await getJSON('coldwell-propuesta:enviados')) || [],
      touch2: [],
    ...
    console.error('Error en cron-seguimiento-coldwell:', e.message);
